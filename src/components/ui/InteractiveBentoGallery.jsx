...
// MediaItem renders either a video or image
  ...
  return (
    <>
      ...
      <motion.div drag dragMomentum={false} dragElastic={0.1}
        initial={false} animate={{ x: dockPosition.x, y: dockPosition.y }}
            ...
            {mediaItems.map((item, index) => (
              ...
              >
                ...
                {selectedItem.id === item.id && (
                  <motion.div layoutId="activeGlow" className="absolute -inset-2 bg-white/20 blur-xl"
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      ...
      <div className="mb-8 text-center">
        <motion.p
          className="text-[#8B1A4A] font-bold uppercase tracking-[0.3em] text-sm mb-3"
          initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          Our Expertise
        </motion.p>
        <motion.h2
          className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-[#2D3A4A] tracking-tight"
          initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          {title}
        </motion.h2>
        <div className="w-14 h-1 bg-[#8B1A4A] mx-auto mt-4 rounded-full" />
        <motion.p
          className="mt-4 text-sm sm:text-base text-gray-500 max-w-xl mx-auto"
      ...
      <AnimatePresence mode="wait">
        {selectedItem ? (
          <GalleryModal selectedItem={selectedItem} isOpen={true}
            onClose={() => setSelectedItem(null)} setSelectedItem={setSelectedItem} mediaItems={items} />
        ) : (
          <motion.div
          ...
          >
            {items.map((item, index) => (
              <motion.div key={item.id} layoutId={`media-${item.id}`}
                className={`relative overflow-hidden rounded-2xl cursor-move shadow-md hover:shadow-xl transition-shadow ${item.span}`}
                ...
                onDragEnd={(e, info) => {
                  ...
                  if (Math.abs(dist) > 50) {
                    ...
                    const targetIdx = dist > 0 ? Math.min(index + 1, items.length - 1) : Math.max(index - 1, 0)
              ...
              >
                ...
                <motion.div className="absolute inset-0 flex flex-col justify-end"
                  ...
                  <div className="relative p-3 sm:p-4">
                    <h3 className="text-white text-xs sm:text-sm font-bold line-clamp-1">{item.title}</h3>
                    <p className="text-white/70 text-[10px] sm:text-xs mt-0.5 line-clamp-2">{item.desc}</p>
                  </div>
                </motion.div>
              </motion.div>
            ))}
